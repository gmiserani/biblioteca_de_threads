...
typedef struct dccthread{
    ...
    dccthread_twaiting_for; //pointer to a thread that it is waiting to finish, NULL otherwise
} dccthread_t;
...
void dccthread_init(void (func)(int), int param){
    ...
    while(!dlist_empty(ready_list)){
        dccthread_tcurrent_thread = (dccthread_t ) dlist_pop_left(ready_list);
...
void dccthread_yield(void){
    dccthread_tcurrent_thread = dccthread_self();
    swapcontext(&manager,&(current_thread->context));
}
